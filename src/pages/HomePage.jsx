...
import { allSortedByDate, pickRotatingLead, sealStatus } from '../data/content';
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <section className="border-b" style={{ borderColor: 'var(--primary)' }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-14 md:py-20 text-center">
          <span className="block text-sm font-semibold mb-6" style={{ color: 'var(--forward)' }}>
            Today's letter
          </span>

          <Link to={leadHref} className="block group">
            <h1 className="font-headline text-[32px] sm:text-[52px] font-semibold leading-[1.1] mb-4 group-hover:underline" style={{ color: 'var(--ink)' }}>
              {lead.headline}
            </h1>
          </Link>
          ...
          <div className="flex items-center justify-center gap-4">
            <div className="flex flex-col items-center gap-1.5">
              <Seal station={lead.station} date={lead.postmark.date} status={sealStatus(lead)} size={92} animate seed={0} />
              <SealDateline station={lead.station} date={lead.postmark.date} />
            </div>
            <Link to={leadHref} className="inline-flex items-center gap-1.5 text-sm font-semibold hover:underline" style={{ color: 'var(--primary)' }}>
              Read the letter
              <ArrowRight className="w-4 h-4" />
            </Link>
          </div>

          {/* The letter's sign-off: the wordmark with the tagline under it,
              below today's headline, per the brief. */}
          <div className="mt-10 pt-6 mx-auto max-w-xs border-t" style={{ borderColor: 'var(--primary)' }}>
            <p className="font-headline text-2xl font-semibold" style={{ color: 'var(--ink)' }}>StoryLettr.com</p>
            <p className="font-headline text-lg italic mt-1" style={{ color: 'var(--ink)', opacity: 0.85 }}>
