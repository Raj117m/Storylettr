...
const PALETTE = [
  "#16120E", // Espresso
  "#1C3D5C", // Sapphire
  "#221B16", // Roasted surface
  "#5C1D24", // Oxblood
  "#2A211B", // Dark walnut
  "#18334D", // Deep sapphire
  "#1B2A38", // Slate navy
  "#4A1A20", // Deep oxblood
  "#CCA352", // Brass
  "#3A3026", // Warm bronze
  "#1F1A15", // Charcoal espresso
  "#25252A",
];
...
function deriveLayout(items: NewsletterBookshelfItem[]) {
  ...
  return items.map<BookLayout>((item) => {
    ...
    const foil =
      item.foil ?? (luminance(color) < 0.5 ? "#CCA352" : "#16120E");
...
function coverTexture(book: BookLayout, brand: string, face: "cover" | "spine") {
  ...
  if (face === "cover") {
    ...
    context.font = "700 52px 'Instrument Serif', Georgia, serif";
    ...
    context.font = "700 37px 'Instrument Serif', Georgia, serif";
...
export function NewsletterBookshelf({
  ...
  return (
    <section
      className={cn(
        "relative isolate w-full overflow-hidden bg-[var(--bg-elevated)] text-[var(--text-primary)] [--shelf-accent:var(--sapphire,#5490C0)]",
        className,
      )}
    ...
    >
      ...
      >
        ...
        <div
          ...
          aria-hidden
          className={cn(
            "pointer-events-none absolute z-20 max-w-[320px] -translate-x-1/2 -translate-y-[calc(100%+16px)] whitespace-nowrap rounded-lg bg-[var(--bg-surface)] border border-[var(--border-light)] px-3.5 py-2 font-mono text-[11px] leading-4 text-[var(--text-primary)] shadow-2xl transition-opacity duration-150 backdrop-blur-md",
            hovered && selectedIndex === null ? "opacity-100" : "opacity-0",
          )}
        >
          <span className="font-semibold block truncate max-w-[280px]">{hovered?.title}</span>
          <span className="block text-[var(--text-muted)] text-[10px] mt-0.5">{hovered?.date}</span>
        </div>
      </div>
    </section>
  );
